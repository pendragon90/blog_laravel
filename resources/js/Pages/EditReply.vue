                ...
                <form @submit.prevent="handleEdit" class="w-full bg-white">
                    <div class="flex flex-col gap-3 border border-slate-400 rounded-xl p-3 mb-5">
                        <div class="flex justify-between">
                            <h1 type="text" class="text-slate-600 font-medium border-none">{{ props.reply.data.user }}
                            </h1>
                            <h1>{{ reply.data.date }}</h1>
                        </div>
                        <input type="text" class="border-none focus:rounded-lg focus:ring-0" v-model="form.content_reply">
                    </div>
...
const props = defineProps(['post', 'comment', 'reply'])

const form = useForm({
    content_reply: props.reply.data.content_reply
})
...
const handleEdit = () => {
    form.transform((data) => ({
        _method: "PUT",
        ...data
    })).post(route('posts.comments.replies.update', [
        props.post.data.slug,
        props.comment.data.slug,
        props.reply.data.slug
    ]))
    form.reset()
}
